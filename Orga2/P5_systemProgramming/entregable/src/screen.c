...
void screen_draw_box(uint32_t fInit, uint32_t cInit, uint32_t fSize,
                     uint32_t cSize, uint8_t character, uint8_t attr) {        // attr es el stributo en VGA, character es el caracter que queres poner, los demas son parametros para los limites.
  // p es un puntero que va recorriendo cada posición de la memoria de video
  ca(*p)[VIDEO_COLS] = (ca(*)[VIDEO_COLS])VIDEO;  //la estructura ca = .character .attr
  uint32_t f;
  uint32_t c;
...
void screen_draw_layout(void) { //es llamado desde kernel asm, pero no hay que hacer pasaje de parametros pq es void
  /* COMPLETAR */
  screen_draw_box(0,0,50,80, 32,0);
  print("MARTIN",25,35,0xF0);
