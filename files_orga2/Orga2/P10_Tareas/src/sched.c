...
typedef struct sched_entry_t {                          //con un arreglo de sched_entry_t se itera entre las tareas
  int8_t task_id;
  int16_t selector;
  bool alive;
} sched_entry_t;

static sched_entry_t sched_tasks[MAX_TASKS] = {0};      //se inicializan todas las tareas en 0
static int8_t last_task = -1;
int8_t current_task = IDLE_TASK_ID;                     // mi id actual representa a current_task
 ...
 */
void sched_add_task(uint16_t selector, int8_t task_id) {               //si quiero agregar una tarea, tengo que sumar al arreglo del scheduler una entrada(selector, alive, id), y ademas tengo que hacer un tss descriptor 
  kassert(selector != 0, "No se puede agregar el selector nulo");      //esta funcion SOLO suma en el arreglo del scheduler
...
void sched_disable_task(int8_t task_id) {
  ...
  for (size_t i = 0; i < MAX_TASKS; i++) {
    if (sched_tasks[i].task_id == task_id) {
      sched_tasks[i].alive = false;           //deshabilita una task con un id del scheduler como parametro
      return;
    }
  }
...
void sched_enable_task(int8_t task_id) {
  ...
  for (size_t i = 0; i < MAX_TASKS; i++) {
    if (sched_tasks[i].task_id == task_id) {
      sched_tasks[i].alive = true;            //aca la revive
      return;
    }
  }
  kassert(false, "Task Not Found!");
...
void sched_init(void) {
  for (size_t i = 0; i < MAX_TASKS; i++) {
    sched_tasks[i].task_id = -1;                 //se inicializa el scheduler, poniendo a todas las entradas en -1. Los -1 representan slots libres
  }
}

uint8_t sched_current_task(){
	return sched_tasks[current_task].task_id;       //devuelve el id de la task actual
}
...
uint16_t sched_next_task(void) {                    // itera entre las tasks para devolver la siguiente que toca. SI estan todas muertas, devuelve la idle.
