 ...
 */
static void create_task(uint8_t task_id, tipo_e tipo, uint8_t indice) {     // le asignas id, tipo, indice para el arerglo task[2][5]
  tss_tasks[task_id] = tss_create_user_task(info_tipo[tipo].code_start); // mete la tss de la tarea nueva en el arreglo de tss

  task[tipo][indice] = (task_t){        // task = arreglo de tareas segun tipo, 2 * 5 ;; tss_tsk es un arreglo de tss por tarea. Et tss_tasks[task_id] esta
      .task_id = task_id,               // el tss de la tarea con id = task_id
      .tipo = tipo,
  };

  size_t gdt_id = (tipo == TASK_A? GDT_IDX_TASK_A_START : GDT_IDX_TASK_B_START) + indice;         // el indice tmb ubica en la gdt
  gdt[gdt_id] = tss_gdt_entry_for_task(&tss_tasks[task_id]); // meto el tss descriptor en gdt
  sched_add_task(gdt_id << 3, task_id);  //meto en el scheduler la tarea nueva (selector y id )
}
...
void task_init(void) { // esto esta en kernel.asm
  ...
  print_title();
   for (uint8_t i = 0; i < MAX_TASKS/2; i++) {
     create_task(i, TASK_A, i);    // pone la tss en el arreglo de tss, pone las esntradas en la gdt, y la pone en el scheduler
     sched_enable_task(i);         //la pone alive en el scheduler
   }
