...
paddr_t shared_memory_paddr[SHARED_MEMORY_SIZE];              // arreglo de direcciones fisicas, esto siempre es memoria compartida
...
void mmu_map_page(uint32_t cr3, vaddr_t virt, paddr_t phy, uint32_t attrs) {
  ...
  pt[pti] = (pt_entry_t){
      .attrs = attrs | MMU_P,
      .page = phy >> 12,           // solo se usa el phy para definir la PTE
  };
  tlbflush();
 ...
 */
paddr_t mmu_init_task_dir(paddr_t phy_start) {         // inicializa directorio de TAREA!  <- tenemos uno que inicializa pero del kernel
  ...
  for (vaddr_t i = 0; i < identity_mapping_end; i += PAGE_SIZE) { // mapea el kernel de nuevo! <- no podes mapear globalmente el kernel.
  ...
  return cr3;
                    ...
                    // Por tarea inicializa el directorio entero, 
